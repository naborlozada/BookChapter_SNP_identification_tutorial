...
It is worth mentioning that *mapping* the reads and perform a *variant calling* procedure using a **single sample** will introduce a lot of bias from a Population Genetics perspective, since one individual cannot be a full representative organism of a entire species when the study it is based only on its genetic variances, particularly from a SNP dataset, same individuals of the sampe species (even from the same sample point/region) might have a considerable number of *private SNPs*. Therefore, it is highly recommended to include more samples in this type of studies that can provide a "consensus" representation of a sampling population region. The number of samples in a region might also depends of the biological question, though. Nevertheless, there is not a *maximum limit on the of samples* reported so far, and it will depends on the requirements of the project, yet, a *thumbs up rule* for a *minimum number of samples* should be 10 individuals. This requirement, however, sometimes cannot be achieved due several reasons (e.g. technical, biological, geographical, economical, others), so we understand that this is an ideal case and sometimes it cannot be possible. We encourage all researches to consider carefully this observation when they start planning the project budjet, goals, and biological questions.

Thus, this single sample protocol steps it is only to illustrate the flow of command lines in a "one-after-another" manner, so it can be easily follow, but also any intrepid person on its own can implement it in a single full script adding and/or modifying elements or steps across this pipeline as they wish, run it automatically in a loop script or in parallel, for example. Check out our scripts for [**Parallelization**](https://github.com/naborlozada/BookChapter_SNP_identification_tutorial/tree/main/Extra_content/parallelization) in the **Extra content** directory section in this repository. You will find basic bash scripts that can be used in a **HPC (High-Performance Computing) cluster system** or in a **Server system** for as many as samples you want to analyze.
